...
Please see below the descriptive statistics of the numerical variables and the correlation matrix.
...
The number of observations is `r sum(!is.na(df$correct))` for all of our key variables.
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Correlation matrix", fig.height=3, fig.width=5, fig.pos="H"}
...
corrplot(matrix,addCoef.col = 'grey50',tl.col = "black",
         tl.cex = 0.7, tl.srt=45,number.cex=0.7, number.digits=2)
...
```{r Boxplot, message=FALSE,warning=FALSE, fig.cap="Boxplot of English correctness", fig.width=7, fig.height=3 }
...
```{r echo=FALSE, fig.cap="Distributions", message=FALSE, warning=FALSE, out.height='100%', out.width='100%', fig.pos="H"}
distribution_figs
```
...
```{r echo=FALSE, message=TRUE, warning=TRUE, fig.cap="Binned Scatterplots"}

ggarrange(bs1)

```

```{r fig.cap="Smoothed patter", warning=FALSE,message=FALSE}
