...
df<-filter(df,df$education %in% educ$education[1:6])
...
#write.table(df,file = "clean_data.csv")
...
theme_gyongyver<-function(base_size=12){
  ...
  theme_light() %+replace% 
    ...
    theme(
      ...
      plot.subtitle = element_text(family = "", colour="midnightblue", size=11, hjust = 0),
      #caption
...
ggplot(df, aes(x=Eng_start, y=elogit))+
  ...
  stat_binscatter(color="red",bins=60,geom = "pointrange")+
  theme_gyongyver()
...
# reg2: With controls
reg2<- feols( correct ~ age + gender + natlangs+ education +psychiatric , data = df , vcov = 'hetero' )
summary(reg2)
...
msummary(list("LPM"=reg1, "Logit"=reg2,"Logit marginal"=reg3,
              "Probit"=reg4,"Probit marginal"=reg5),
